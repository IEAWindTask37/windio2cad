...
def write_openscad(lofted_shape):
    ...
    for k in range(0, n_span - 1, 10):
        bottom = float(lofted_shape[k, 0, 2])
        top = float(lofted_shape[k + 1, 0, 2])
        points_lower = [[row[0], row[1], bottom] for row in lofted_shape[k, :, :]]
        points_upper = [[row[0], row[1], top] for row in lofted_shape[k, :, :]]
