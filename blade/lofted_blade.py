...
def generate_lofted(outer_shape, airfoils, n_span_min=100, n_xy=400):
    ...
    ref_axis   = np.c_[myinterp(r_span, outer_shape['reference_axis']['x']['grid'], outer_shape['reference_axis']['x']['values']),
                       myinterp(r_span, outer_shape['reference_axis']['y']['grid'], outer_shape['reference_axis']['y']['values']),
                       myinterp(r_span, outer_shape['reference_axis']['z']['grid'], outer_shape['reference_axis']['z']['values'])]
    ...
    coord_xy_interp = np.flip( myinterp(np.flip(r_thick_interp), r_thick_unique, coord_xy_used[indices, :, :]), axis=0)
    for i in range(n_span):
        ...
        if r_thick_interp[i] < 0.4:
            coord_xy_interp[i,:,:] = geom.trailing_edge_smoothing(coord_xy_interp[i,:,:])
    ...
    for i in range(n_span):
        for j in range(n_xy):
            lofted_shape[i,j,:] = np.r_[coord_xy_dim_twisted[i,j,1], coord_xy_dim_twisted[i,j,0], 0.0] + ref_axis[i,:]
...
def write_openscad(lofted_shape):
    ...
    for k in range(n_span):
        ...
        # Graph version
        ax.plot(lofted_shape[k,:,0], lofted_shape[k,:,1], lofted_shape[k,:,2],'b')
